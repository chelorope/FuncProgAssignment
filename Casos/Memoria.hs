...
module Memoria (iniciar, actualizarLinea, insertar, acumular, obtener, pertenece) where  
import Data.Map (Map)
import AwkiSA
...
actualizarLinea s map = M.insert "$0" (Str s) (M.insert "NR" ((+) (obtener "NR" map) 1) (M.insert "NF" (Num (length (words s))) (actualizarPesos (length (words s)) (words s) map)))
