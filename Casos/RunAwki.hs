...
runAwkiSt prog (s:sx) map =  if (toBool (obtener "exit" map) == False) then (concatv res resRec, mapRec) else (Str "", map)
	where
...
recorrerProg (p:px) s map = if (toBool (obtener "exit" map) == False) then (if fst resp then  
...
evaSequence (st:rst) map = if (toBool (obtener "exit" map) == False) then (concatv resPrint resRecPrint, concatv res resRec, mapRec) else (Str "", Str "", map)
	where 
...
evaExpr (Field exp) map = if (fst (evaExpr exp map)) >= (Num 0) then (obtener ("$" ++ show (fst (evaExpr exp map))) map, map) else error "ERROR: Campo negativo"
---------------
evaOp2 :: BOp -> Expr -> Expr -> Map String Valor -> (Valor, Map String Valor)
evaOp2 Div p1 p2 map = if toBool (fst exp2) then (((evaBOp Div) (fst exp1) (fst exp2)), (snd exp2)) else error "ERROR: Division ente 0"
	where 
		exp1 = evaExpr p1 map
		exp2 = evaExpr p2 (snd exp1)
evaOp2 Mod p1 p2 map = if toBool (fst exp2) then (((evaBOp Mod) (fst exp1) (fst exp2)), (snd exp2)) else error "ERROR: Division ente 0"
	where
		exp1 = evaExpr p1 map
		exp2 = evaExpr p2 (snd exp1)
